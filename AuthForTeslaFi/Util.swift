...
func logRequestEvent(message: String) {
    ...
    if let requestEventLogJson = UserDefaults.standard.data(forKey: kRequestEventLog),
       let requestEventLog = try? JSONDecoder().decode([RequestEvent].self, from: requestEventLogJson)
    {
